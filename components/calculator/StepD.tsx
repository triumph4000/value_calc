  ...
  return (
    <button
      ...
      className={`w-full text-left p-4 rounded-xl border transition-all duration-200 ${
        value
          ? 'bg-[#17C662]/5 border-[#17C662]/40'
          : 'bg-[#0D0D0D] border-[#2A2A2A] hover:border-[#3A3A3A]'
      }`}
    >
      <div className="flex items-center justify-between gap-4">
        <div className="flex-1 min-w-0">
          <p className={`text-sm font-bold ${value ? 'text-white' : 'text-white/70'}`}>
            {question}
          </p>
          <p className="text-xs text-[#6B7280] mt-0.5 font-medium">{detail}</p>
        </div>
        <div className="flex-shrink-0">
          <div
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 ${
              value ? 'bg-[#17C662]' : 'bg-[#2A2A2A]'
            }`}
          >
            <span
...
const MATURITY_CONFIG = [
  { label: 'Reactive',      color: 'text-red-400',       bar: 'bg-red-500',          pct: '5%'   },
  { label: 'Reactive',      color: 'text-red-400',       bar: 'bg-red-500',          pct: '25%'  },
  { label: 'Developing',    color: 'text-amber-400',     bar: 'bg-amber-500',        pct: '50%'  },
  { label: 'Proactive',     color: 'text-[#17C662]',     bar: 'bg-[#17C662]',        pct: '75%'  },
    ...
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-black text-white mb-1 tracking-tight">
          Renewal & Expansion Sophistication
        </h2>
      ...
      <div className="space-y-3">
        ...
        <ToggleQuestion
          ...
          detail="Quarterly Business Reviews scheduled and conducted on a regular cadence"
          value={data.runsQBRs}
        ...
        <ToggleQuestion
          ...
          detail="Someone whose primary role is retention and expansion — not just support"
      ...
      <div className="bg-[#0D0D0D] border border-[#2A2A2A] rounded-xl p-5">
        <div className="flex items-center justify-between mb-3">
          <p className="text-sm font-bold text-white/70">Operational Maturity Score</p>
        ...
        <div className="h-2.5 bg-[#2A2A2A] rounded-full overflow-hidden mb-3">
          <div
            className={`h-full rounded-full transition-all duration-500 ${maturity.bar}`}
            style={{ width: maturity.pct }}
          />
        </div>
